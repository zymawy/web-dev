 ...
 * The Footer Partial
 */
Handlebars.registerPartial('search',
    ...
    <p class="uk-article-meta">The Result was found in 15202252000</p>
    <br class="clear">
    {{#each searchResults }}
    <div>
...
document.addEventListener('news-data-loaded', ({ detail }) => {
    ...
    const containerSearch = $('#container-search'),
        searchNewTemplate = $('#search-template');
    const searchTemplateCompiled = Handlebars.compile(
        searchNewTemplate.html()
    );

    console.log(searchResults);
    containerSearch.empty().append(searchTemplateCompiled({ searchResults }));
